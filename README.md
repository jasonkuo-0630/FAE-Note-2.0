...
- 左側採用可展開／收合的階層式導覽。
- `GV-VMS` 已建立 12 個第二層章節。
- 點擊 `GV-VMS` 名稱會顯示全部筆記；點擊章節只顯示該章節。
- 同一時間只展開一個領域，並保留目前展開與選取位置。
- 主畫面上方保留筆記類型與狀態篩選。
- 目前仍是空資料版本，正式筆記數量為 0。
- 未納入內部的 `Camera testing process`。
...
- 想改 GV-VMS 第二層章節：`data/taxonomy.js`
